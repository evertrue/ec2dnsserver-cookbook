...
describe Chef::Recipe::Ec2DnsServer do
  ...
  # describe '#server_ip_by_hostname' do
  #   let(:helpers) { Chef::Recipe::Ec2DnsServer.new('stage', test_zone_apex) }
  ...
  #   context 'target server exists' do
  #     let(:server) do
  #       server = fog_conn.servers.create
  #       server.wait_for { ready? }
  #       server
  ...
  #     before(:each) do
  #       fog_conn.create_tags(server.id, 'Name' => 'some-node')
  #       private_interface_id = fog_conn.create_network_interface(
  #         private_subnet_id,
  #         'PrivateIpAddress' => private_ip
  #       ).data[:body]['networkInterface']['networkInterfaceId']
  ...
  #     it 'requests the IP of the mock server' do
  #       expect(helpers).to receive(:server_obj_ip).with(server)
  #         .and_return(private_ip)
  #       helpers.send(:server_ip_by_hostname, 'some-node')
  #     end

  #     it 'returns the IP of the mock server' do
  #       allow(helpers).to receive(:server_obj_ip).with(server)
  #         .and_return(private_ip)
  #       expect(helpers.send(:server_ip_by_hostname, 'some-node'))
  #         .to eq(private_ip)
  #     end
  #   end

  #   context 'target server does not exist' do
  #     it 'raises an exception' do
  #       expect { helpers.send(:server_ip_by_hostname, 'some-node') }
  #         .to raise_error(
  #           RuntimeError,
  #           "Can't locate server with name some-node"
  #         )
  #     end
  #   end
  # end
